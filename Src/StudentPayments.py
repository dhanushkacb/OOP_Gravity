...
class StudentPayments(BaseRegistration):
    ...
    def __init__(self, entity_name="Payment"):
        ...
        # Amount
        tk.Label(self.form_frame, text="Amount:", anchor="w").grid(row=5, column=0, sticky="w", padx=5, pady=5)
        ...
        # Payment Method
        tk.Label(self.form_frame, text="Payment Method:", anchor="w").grid(row=6, column=0, sticky="w", padx=5, pady=5)
        self.payment_method_var = tk.StringVar()
        self.payment_method_menu = ttk.Combobox(
            self.form_frame,
            textvariable=self.payment_method_var,
            values=self._settings.get_payment_methods() if hasattr(self._settings, "get_payment_methods") else ["Cash", "Card", "Online"],
            width=27,
            state="readonly"
        )
        self.payment_method_menu.grid(row=6, column=1, padx=5, pady=5, sticky="ew")

        # Remarks
        tk.Label(self.form_frame, text="Remarks:", anchor="w").grid(row=7, column=0, sticky="w", padx=5, pady=5)
        ...
        columns = ("payment_id", "student", "class", "month", "year", "amount", "method", "remarks", "delete")
    ...
    def save_record(self, event=None):
        try:
            ...
            if self._model.insert(student_id, class_id, month, year, amount, method, remarks):
    ...
    def on_tree_select(self, event):
        ...
        self.payment_method_var.set(values[6])
        self.remarks_entry.delete(0, tk.END)
        self.remarks_entry.insert(0, values[7])
    ...
    def update_record(self, event=None):
        try:
            ...
            if self._model.update(self.selected_key, student_id, class_id, month, year, amount, method, remarks):
